...
<section class="relative py-6 bg-cream" x-data="eventSlider()">
    ...
    <div class="relative z-20 max-w-7xl mx-auto px-6 qhd:max-w-[1980px]">
        ...
        <div class="relative w-full h-112.5 md:h-137.5 lg:h-162.5 group">
            ...
            <button @click="prev()" class="absolute left-4 top-1/2 -translate-y-1/2 z-40 w-10 h-10 md:w-14 md:h-14 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white flex items-center justify-center hover:bg-ev-dark transition-all shadow-lg active:scale-95">
                <svg width="24" height="24" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7" /></svg>
            </button>

            <button @click="next()" class="absolute right-4 top-1/2 -translate-y-1/2 z-40 w-10 h-10 md:w-14 md:h-14 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white flex items-center justify-center hover:bg-ev-dark transition-all shadow-lg active:scale-95">
                ...
                <div class="relative rounded-3xl overflow-hidden shadow-lg h-full border border-gray-100">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 px-6 py-4 bg-white/80 uppercase">
                        ...
                        <small class="text-gray-600 text-xs flex items-center gap-1">
                            <i class="fa-solid fa-location-dot text-red-500"></i> {{ $item['location'] }}
                        </small>
                    </div>
                </div>
            </div>
        ...
        @foreach($editions as $edition)
            <div class="flex flex-col {{ $edition['rev'] ? 'md:flex-row-reverse' : 'md:flex-row' }} items-stretch rounded-3xl bg-white mb-10 overflow-hidden shadow-xl border border-gray-200">
                <div class="w-full lg:w-[40%] md:w-[45%] shrink-0">
                    <img src="{{ asset('storage/images/' . $edition['img']) }}" class="w-full h-full object-cover">
                </div>
                <div class="flex-1 p-8 md:p-12 flex flex-col justify-center">
                    <h3 class="font-semibold text-2xl uppercase mb-6">Еволуција на Сонот {{ $edition['id'] }}</h3>
                    <div class="space-y-8">
